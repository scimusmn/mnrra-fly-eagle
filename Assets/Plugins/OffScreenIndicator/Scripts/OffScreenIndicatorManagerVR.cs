	...
	public class OffScreenIndicatorManagerVR : OffScreenIndicatorManager {
		...
		protected override void UpdateIndicatorPosition(ArrowIndicator arrowIndicator, int id = 0){
			...
			if (plane.Raycast(rToTarget,out distance)){
				...
				//If Target-cast doesn't hit the plane. Indicator will be drawn in the radius
				//1. obtain only the angle when casting the target alongside plane normal
				//2. move indicator in the angle direction to radius
				//3. rotate indicator that angle
				//Debug.Log("Plane not hit");
			}
		}
	}
}